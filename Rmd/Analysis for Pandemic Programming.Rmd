...
Let's start by loading `lavaan`, for developing our structural equation model (SEM), `data.table`, for loading our data, and `semPlot` in case we want to plot our SEM.
...
Gender has way too few data points outside male/female so keep only those. Remove 18 cases (16 "Not answering", and 2 "Other")
```{r}
d <- d[d$Gender == "Female" | d$Gender == "Male", ]
nrow(d)
```
The data we will use will be stored in a new data frame `df`, so that we keep the original data ($n=2194$) untouched. All predictors will be in small letters and the suffixes on the variable names (always good practice to do this) will be:
...
So we have $n=1377$, a number of predictors and outcomes, and they should now be correctly formatted.

## Non-identifiability

For all our predictors, and the corresponding outcomes, we analyzed non-identifiability. It would take too much space to show all these steps but, in short, for each outcome check the corresponding covariates by creating a model matrix (without the intercept).

The model matrix is then QR decomposed and we check for non-identifiability on the decomposed matrix. This can be done qualitatively, by examining pair plots.^[https://www.martinmodrak.cz/2018/05/14/identifying-non-identifiability/] However, one can also do this quantitatively.^[https://discourse.mc-stan.org/t/blog-post-identifying-non-identifiability/4201/3]
...
The summary shows that $\chi^2$, `CFI/TLI`, `SRMR` look ok. For $\chi^2$ we want the test statistic for the `Model Test User Model` divided by the number of free parameters to be $\gt 2$. `CFI/TLI` should all $\gt 0.95$. `SRMR` and `RMSEA`should be $\lt 0.05$.^[https://paolotoffanin.wordpress.com/2018/05/19/beginning-with-sem-in-lavaan/]

`RMSEA` is slightly above the threshold but that is not strange considering our sample size $n=1377$.
...
and our factor loadings, which should be $0$,
...
```
We see some diagnostics starting to move away from the thresholds, but this is the full model so not strange. Let's remove all covariates with $p \gt 0.1$,
...
To summarize, `SRMR`$\lt 0.8$  and `RMSEA` around $0.051$, is generally considered a good fit given our sample size [@HuB99sem]. `CFI/TLI` are both $\gt 0.95$.

Now one can conduct inferences on our SEM.
