...
The summary shows that $\chi^2$, `CFI/TLI`, `SRMR` look ok. For $\chi^2$ we want the test statistic for the `Model Test User Model` divided by the number of free parameters to be $\gt 2$. `CFI/TLI` should be $\gt 0.95$ and `SRMR` and `RMSEA`should be $\lt 0.05$.^[https://paolotoffanin.wordpress.com/2018/05/19/beginning-with-sem-in-lavaan/]

`RMSEA` is slightly above the cutoff criteria but that is not strange considering our sample size $n=1377$.
...
To summarize, `SRMR`$\lt 0.8$  and `RMSEA`$= 0.051$, is generally considered a good fit given our sample size [@HuB99sem]. `CFI/TLI` are both $\gt 0.95$.
